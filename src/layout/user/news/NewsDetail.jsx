...
export default function NewsDetail() {
  ...
  const detailnews = NewsData?.news.find((element) => element.id == id);
  const otherNews = NewsData?.news.filter((element) => element.id != id);
              ...
              <div className="lg:flex flex-1">
                <div className="lg:w-9/12 w-full lg:px-0 px-6">
                  ...
                  <div className="text-gray-400 text-center text-base mb-2">
                    {dayjs(detailnews?.created_at).format(
                      "dddd, MMM D, YYYY h:mm A"
                    )}
                  </div>
                  <img
                    className="h-full lg:max-h-[500px] w-full my-3 rounded-2xl lg:w-full flex-none object-cover object-center mx-auto text-center overflow-hidden"
                ...
                <div className="lg:w-3/12 w-full lg:mx-4 lg:px-0 px-3">
                  <div className="grid grid-cols-1">
                    <Link to="/news">
                      <h1 className="text-blue-600 lg:text-xl md:text-xl text-lg font-semibold hover:text-blue-400 my-4">
                        Baca juga :
                      </h1>
                    </Link>
                  ...
                  <ul class="max-w-md space-y-1 text-gray-500 font-medium lg:text-base mg:text-base text-sm dark:text-gray-400 pb-3">
                    {data?.events.slice(0, 3).map((program) => (
