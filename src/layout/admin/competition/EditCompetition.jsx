...
import { GraphQLUpdateCompetitionById, GraphQlCompetition } from "../../../graphql/GraphQlCompetition";
...
export default function EditCompetition() {
  ...
  const {UpdateCompetition,LoadingUpdate,LoadingError} = GraphQLUpdateCompetitionById()
  ...
  useEffect(() => {
    if (CompetitionData) {
      const competition = CompetitionData?.competition.find((element) => element.id == id);
      const dataInialisasi = {
        name: competition.name,
      };
      
      // Convert the date to the format supported by datetime-local input
      const formattedStartDate = dayjs(competition.date).format(
        "YYYY-MM-DD"
      );
      formik.setValues({
        ...dataInialisasi,
       date: formattedStartDate, // Set the default value for date
      });
    }
  }, [CompetitionData]);
  if (CompetitionError) return <ErrorPage />;
